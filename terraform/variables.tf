...
variable "step_stay_stopped_stack_name_suffix" {
  type        = string
...
variable "step_stay_stopped_params" {
  type = object({
...
variable "step_stay_stopped_tags" {
  type        = map(string)
  ...
  validation {
    ...
    condition = alltrue([
      for value in values(var.step_stay_stopped_tags) : try(length(value) >= 1, true)
    ])
...
variable "step_stay_stopped_region" {
  type        = string
